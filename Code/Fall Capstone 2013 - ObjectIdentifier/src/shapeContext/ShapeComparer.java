...
public class ShapeComparer {
	...
	private void computeCostMatrix(){
		...
		for(Point p: unknownHistograms.keySet()){
			...
			for(Point q: knownHistograms.keySet()){
				...
				for(int k = 0; k < unknownHistograms.get(p).getNumBins(); k++){
					...
					totalCost += 
							(Math.pow((unknownHistograms.get(p).getBinCount(k)/unknownHistograms.get(p).getTotalBinCount())-(knownHistograms.get(q).getBinCount(k)/knownHistograms.get(q).getTotalBinCount()),2))/
							((unknownHistograms.get(p).getBinCount(k)/unknownHistograms.get(p).getTotalBinCount())+(knownHistograms.get(q).getBinCount(k)/knownHistograms.get(q).getTotalBinCount()));
				}
		...
		Iterator<Point> pointIterator = shapeContextCost.keySet().iterator();
		
		for(int row = 0; row < shapeContextCost.size(); row++){
			Iterator<Point> otherPointIterator = shapeContextCost.keySet().iterator();
	...
	public List<Line> getMinLines(double[][] matrix) {
	...
	private double[][] findMatching(){
		int[] rowZeros = new int[costMatrix.length];
		int[] colZeros = new int[costMatrix[0].length];
		double[][] result = new double[costMatrix.length][costMatrix[0].length];
